...
void copy(R_in&& in, R_out&& out) {
  ...
  parallel_for(0, parlay::size(in),
               [in_it = std::begin(in), out_it = std::begin(out)](size_t i) { out_it[i] = in_it[i]; });
}
...
//  r[0] + r[1] + ... + r[n-1]
template<typename R, typename Monoid, std::enable_if_t<is_monoid_v<Monoid>, int> = 0>
...
auto pack_into(R_in&& in, BoolSeq&& b, R_out&& out) {
  ...
  static_assert(std::is_convertible_v<range_reference_type_t<BoolSeq>, bool>);
  static_assert(
      std::is_constructible_v<std::remove_reference_t<range_reference_type_t<R_out>>, range_reference_type_t<R_in>>);
...
size_t count_if_index(size_t n, IntegerPred&& p) {
  ...
  auto BS = delayed_tabulate(n, [&](size_t i) -> size_t { return static_cast<bool>(p(i)); });
...
auto adjacent_find(R&& r, BinaryPred&& p) {
  ...
  auto idx =
      internal::find_if_index(parlay::size(r) - 1, [&p, it = std::begin(r)](size_t i) { return p(it[i], it[i + 1]); });
  if (idx == parlay::size(r) - 1)
    return std::end(r);
  else
    return std::begin(r) + idx;
}
...
auto unique(R&& r, BinaryPredicate&& eq) {
  ...
  static_assert(std::is_constructible_v<range_value_type_t<R>, range_reference_type_t<R>>);
  auto b = internal::delayed_tabulate(
      parlay::size(r), [&eq, it = std::begin(r)](size_t i) { return (i == 0) || !eq(it[i], it[i - 1]); });
...
auto reverse(R&& r) {
  ...
  return internal::tabulate<range_value_type_t<R>>(n, [n, it = std::begin(r)](size_t i) { return it[n - i - 1]; });
}
...
auto rotate(const R& r, size_t t) {
  ...
  static_assert(std::is_constructible_v<range_value_type_t<R>, range_reference_type_t<R>>);
  size_t n = parlay::size(r);
  return internal::tabulate<range_value_type_t<R>>(parlay::size(r), [n, t, it = std::begin(r)](size_t i) {
...
auto remove_if(R&& r, UnaryPred&& pred) {
  ...
  static_assert(std::is_invocable_r_v<bool, UnaryPred, range_reference_type_t<R>>);
  static_assert(std::is_constructible_v<range_value_type_t<R>, range_reference_type_t<R>>);
  auto flags = delayed_seq<bool>(parlay::size(r), [&pred, it = std::begin(r)](size_t i) { return !pred(it[i]); });
...
auto flatten(R&& r) {
  ...
  parallel_for(0, parlay::size(r), [&, it = std::begin(r)](size_t i) {
    ...
    parallel_for(0, parlay::size(inner), [=](size_t j) { assign_uninitialized(*(dit + j), *(sit + j)); }, 1000);
  });
  return res;
}
...
auto map_tokens_small(R&& r, UnaryOp&& f, UnaryPred&& is_space = is_whitespace) {
  ...
  sequence<size_t> Locations = internal::delayed::to_sequence(
      internal::delayed::filter_op(parlay::iota<size_t>(n + 1), [&](size_t i) -> std::optional<size_t> {
  ...
  // and return a sequence of the resulting values
    ...
    return tabulate(Locations.size() / 2, [&](size_t i) { return f(S.cut(Locations[2 * i], Locations[2 * i + 1])); });
  }
}
}  // namespace internal
...
auto map_tokens(R&& r, UnaryOp&& f, UnaryPred&& is_space = is_whitespace) {
  ...
  auto in =
      internal::delayed_tabulate(n + 1, [&](size_t i) -> ipair { return is_start(i) ? ipair(1, i) : ipair(0, 0); });
...
sequence<parlay::chars> tokens(Range&& R, UnaryPred&& is_space = is_whitespace) {
  ...
  if (parlay::size(R) < 2000)
    return internal::map_tokens_small(R, to_token, is_space);
  else
    return map_tokens(R, to_token, is_space);
}
...
auto remove_duplicates_ordered(R&& s, Compare&& less = {}) {
  ...
  static_assert(std::is_invocable_r_v<bool, Compare, range_reference_type_t<R>, range_reference_type_t<R>>);
  static_assert(std::is_constructible_v<range_value_type_t<R>, range_reference_type_t<R>>);
  return unique(stable_sort(s, less), [&](auto&& a, auto&& b) { return !less(a, b) && !less(b, a); });
}
...
auto rank(Range&& r, BinaryOperator&& compare = {}) {
  static_assert(is_random_access_range_v<Range>);
  static_assert(
      std::is_invocable_r_v<bool, BinaryOperator, range_reference_type_t<Range>, range_reference_type_t<Range>>);
  ...
  parallel_for(0, position.size(), [&](size_t i) { rank[position[i]] = i; });
  return rank;
}

template<typename Range, typename BinaryOperator = std::less<>>
auto rank(Range&& r, BinaryOperator&& compare = {}) {
  return parlay::rank<std::make_unsigned_t<range_difference_type_t<Range>>>(std::forward<Range>(r),
                                                                            std::forward<BinaryOperator>(compare));
}
...
auto kth_smallest_copy(Range&& in, size_t k, Compare&& less = {}) {
  ...
  auto pivots = parlay::sort(parlay::tabulate(sample_size * over,
                                              [&](size_t i) -> range_value_type_t<Range> {
                                                auto r = gen[i];
                                                return it[dis(r)];
                                              }),
                             less);
  ...
  auto ids = parlay::tabulate(n, [&](size_t i) -> unsigned char { return ss.rank(it[i], less); });
...
auto kth_smallest(Range&& in, size_t k, Compare&& less = {}) {
  ...
  return kth_smallest_copy(iterators, k,
                           [less = std::forward<Compare>(less)](auto&& it1, auto&& it2) { return less(*it1, *it2); });
}
